...
posicion_gato1 = (random.randint(0,1),random.randint(0,1))
posicion_raton1 = ( random.randint(3,5),random.randint(3,5))
random_nrgato = random.randint(3,7)
...
def movimiento_raton(tamanho_tablero, posicion_gato, posicion_raton, aleatorio):
    if aleatorio : 
        movs = []
        for movimiento in obtener_posibles_movimientos(tamanho_tablero, posicion_raton):
            movs.append(movimiento)
        mjr_mov = random.choice (movs)
    else :
        mjr_mov = None 
        mjr_valor = float("-inf")
        for movimiento in obtener_posibles_movimientos(tamanho_tablero, posicion_raton):
...
while True:
    ...
    total_movientos.append (posicion_gato)
    actualizar_tablero(posicion_raton, posicion_gato)
    ...
    posicion_raton = movimiento_raton (tamanho_tablero,posicion_gato,posicion_raton,forzar)
    total_movientos.append(posicion_raton)
    actualizar_tablero (posicion_raton,posicion_gato)
    if posicion_gato == posicion_raton:
        print ('el gato a atrapado al raton')
        break 
    ...
    if len(total_movientos) > 10:
        total_movientos.pop(0)
        filtro = {}
        for movimiento1 in total_movientos:
            if movimiento1 in filtro:
                filtro[movimiento1] += 1
            else :
                filtro[movimiento1] = 1
            if filtro [movimiento1] > 3 :
                forzar = True
    cont +=1
    if cont == 15:
